...
<script type="text/javascript">
	...
	function search() {
		...
		pageForm.action="IndentQueryServlet.do?oper=list";
	...
	function del(){
		...
		var d = dialog({
		    ...
		    ok: function () {
				...
				pageForm.action="IndentUpdateServlet.do?oper=delete";
				pageForm.method="post";
				pageForm.submit(); 
		        return false;
		    },
									...
									<div class="form-group">
										<div class="row">
											<div class="col-md-2 grid_box1">
												...
												<input type="text" class="form-control1" placeholder="账单号" name="indent_code" value="${indent_code1 }">
											</div>
											...
											<div class="col-md-3 grid_box1" style="margin-top:10px;">
												<span style="font-size: 20px;font-family:楷体; color:green;">金额:${payment }</span>
											</div>
										</div>
										</div>
							</form>
							...
							<tbody>
								<c:forEach items="${list }" var="stu" varStatus="vs">
									<tr id="${vs.count }">
										...
										<td>${stu.indent_code }</td>
										<td><a href="CustomerQueryServlet.do?oper=query&id=${stu.idC }" >${stu.nameC }</a></td>
										<td><a href="MovieQueryServlet.do?oper=query&id=${stu.movie_id}" >${stu.name }</a></td>
										<td><a href="HallQueryServlet.do?oper=query&id=${stu.hall_id }" >${stu.hall_id }</a></td>
										<td><a href="ShowMessageQueryServlet.do?oper=query&idE=${stu.message_id }" >${stu.message_id }</a></td>
										<td>第${stu.seat1 }排 第${stu.seat2 }列</td>
										<td>${stu.payment}</td>
										 <%-- <td>
									 	 <a href="IndentQueryServlet.do?oper=query&id=${stu.id }" >
									 	 编辑
									 	</a>
										 	<form id="pageFormDel">
